class DealsController < ApplicationController
  ...
  def new
    ...
    @deal.currency = @deal.currency || RedmineContacts.settings[:default_currency].to_i
  ...
  def find_deals(pages=true)  
    ...
    scope = Deal.scoped({}) 
    scope = scope.scoped(:conditions => ["#{Deal.table_name}.project_id = ?", @project.id]) if @project
    scope = scope.scoped(:conditions => ["#{Deal.table_name}.status_id = ?", params[:status_id]]) if (!params[:status_id].blank? && params[:status_id] != "o") && !(@project && !@project.deal_statuses.any?) && DealStatus.any?
    ...
    scope = scope.visible
    scope = scope.scoped(:include => :status, :conditions => ["#{DealStatus.table_name}.is_closed = ?", false]) if (params[:status_id] == "o" && !(@project && !@project.deal_statuses.any?)) && DealStatus.any?
