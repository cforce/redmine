class ContactsController < ApplicationController
  ...
  def index   
    ...
    respond_to do |format|   
      ...
      format.js { render :partial => RedmineContacts.list_partial, :layout => false } 
      format.api
  ...
  def bulk_edit
    ...
    raise ActiveRecord::RecordNotFound if @contacts.empty?
    @tag_list = ActsAsTaggableContacts::TagList.from(@contacts.map(&:tag_list).inject{|memo,t| memo | t})
  ...
  def bulk_update
    ...
    @contacts.each do |contact|
      contact.reload
      params[:contact][:tag_list] = (contact.tag_list + ActsAsTaggableContacts::TagList.from(params[:add_tag_list]) - ActsAsTaggableContacts::TagList.from(params[:delete_tag_list])).uniq
...
private      
  ...
  def find_tags     
    ...
    options[:group] = "#{ActsAsTaggableContacts::Tag.table_name}.id, #{ActsAsTaggableContacts::Tag.table_name}.name, #{ActsAsTaggableContacts::Tag.table_name}.created_at, #{ActsAsTaggableContacts::Tag.table_name}.updated_at, #{ActsAsTaggableContacts::Tag.table_name}.color"
  ...
  def find_contacts(pages=true)   
    @tag = ActsAsTaggableContacts::TagList.from(params[:tag]).map{|tag| ActsAsTaggableContacts::Tag.find_by_name(tag) } unless params[:tag].blank? 
