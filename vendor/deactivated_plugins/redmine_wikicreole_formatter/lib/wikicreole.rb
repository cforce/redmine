...
# 
# This software was written by Gordon McCreight (mailto:wikicreole.to.gordon@mccreight.com),
# with some modifications by Brant Young (mailto:brant@9thsoft.com)
# for support Redmine builtin wiki.
#
# Author::    Gordon McCreight  (mailto:wikicreole.to.gordon@mccreight.com)
#             Brant Young (mailto:brant@9thsoft.com)
# Copyright:: Copyright (c) 2008 Gordon McCreight
#             Copyright (c) 2009 Brant Young
...
class WikiCreole
  ...
  def self.creole_wikilink(&blk)
    @wikilink_function = blk
  end
  ...
  def self.creole_customwikilinks
    @@chunks_hash[:wikipath][:open] = ""
    @@chunks_hash[:wikipath][:close] = ""
    @@chunks_hash[:wikilink][:open] = ""
    @@chunks_hash[:wikilink][:close] = ""
    @@chunks_hash[:wikilink].delete(:contains)
    @@chunks_hash[:wikilink][:filter] = Proc.new {|s|
      s = @wikilink_function.call(s) if @wikilink_function
      s
    }
  end
...
private
  ...
  INLINE       = %w{strong em br esc img inowiki link wikilink ilink 
                    sub sup mono u plug plug2 tm reg copy ndash ellipsis amp}

  ALL_INLINE   = [INLINE, 'plain', 'any'].flatten # including plain text

  BLOCKS       = %w{h1 h2 h3 hr nowiki h4 h5 h6 ul ol table p ip bq dl plug plug2 blank}
  ...
  @@chunks_hash = {
    ...
    :p => {
      :curpat => '(?=.)',
      :stops => ['blank', 'ip', 'bq', 'h', 'hr', 'nowiki', 'ul', 'ol', 'dl', 'table'],
      :hint => PLAINCHARS,
      :contains => ALL_INLINE,
    ...
    :ip => {
      :curpat => '(?=:)',
      :fwpat => '\n(?=:)',
      :stops => ['blank', 'h', 'hr', 'nowiki', 'bq', 'ul', 'ol', 'dl', 'table'],
      :hint => [':'],
      :contains => ['p', 'ip'],
      :filter => Proc.new {|s|
        s.sub!(/:/, '')
    ...
    :bq => {
      :curpat => '(?=\>)',
      :fwpat => '\n(?=\>)',
      :stops => ['blank', 'h', 'hr', 'nowiki', 'ip', 'ul', 'ol', 'dl', 'table'],
      :hint => ['>'],
      :contains => ['p', 'bq'],
      :filter => Proc.new {|s|
        s.sub!(/\>/, '')
    ...
    :ul => {
      :curpat => '(?=(?:`| *)\*[^*])',
      :fwpat => '(?=\n(?:`| *)\*[^*])',
      :stops => ['blank', 'ip', 'bq', 'h', 'nowiki', 'li', 'table', 'hr', 'dl'],
      :contains => ['ul', 'ol', 'li'],
      :hint => ['*', ' '],
      :filter => Proc.new {|s|
        s = strip_list(s)
    ...
    :wikipath => {
      ...
      :filter => Proc.new {|s|
        s.sub!(/^\s*/, '')
        s.sub!(/\s*$/, '')
        if @wikilink_function
          s = @wikilink_function.call(s)
        end
        s
      },
      :open => '', :close => '',
